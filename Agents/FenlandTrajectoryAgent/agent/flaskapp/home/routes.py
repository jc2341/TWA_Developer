...
#                     }
#                 };
#                 var data = JSON.stringify({"file_path": "/app/agent/raw_data/gps_target_folder"});
#                 xhr.send(data);
#             }
#         </script>
#     </head>
#     <body>
#         <B>Fenland GPS Trajectory Agent offers the following functions via the specified API endpoints:</B><BR>
#         (Tasks are processed step by step, and you might need to trigger each step separately)<BR><BR><BR>
#         Process and instantiate GPS trajectory data:<BR>
#         This service allows for the step-by-step processing and instantiation of GPS data in the target folder collected from Fenland, UK. Below are the steps and corresponding HTTP requests to process and instantiate your GPS data:<BR><BR>
#         &nbsp;&nbsp;1. <B>[POST request]</B> Load and preprocess GPS CSV files in the target folder:<BR>
#         &nbsp;&nbsp;&nbsp;&nbsp;<button onclick="sendPostRequest('/gpstasks/fenlandtrajectoryagent/load_and_preprocess')">Load and Preprocess GPS CSV</button><BR><BR>
#         &nbsp;&nbsp;2. <B>[POST request]</B> Instantiate processed GPS data into the Knowledge Graph (KG) and PostGIS database:<BR>
#         &nbsp;&nbsp;&nbsp;&nbsp;<button onclick="sendPostRequest('/gpstasks/fenlandtrajectoryagent/process_and_instantiate')">Instantiate GPS Data</button><BR><BR>
#         Note: The current implementation requires manual placement of .csv files into the target folder. Future updates may include direct upload functionality through this web interface.<BR><BR>
#         For more detailed instructions or assistance, please refer to the documentation or contact the support team.<BR>
#     </body>
#     </html>
#     """
...
def home():
    ...
    <body>
        ...
        <button class="button" onclick="sendPostRequest('/gpstasks/fenlandtrajectoryagent/load_and_preprocess')">Load and Preprocess GPS CSV</button>
        
        <h2>2. [POST request] Instantiate processed GPS data into the Knowledge Graph:</h2>
        <button class="button" onclick="sendPostRequest('/gpstasks/fenlandtrajectoryagent/process_and_instantiate')">Instantiate GPS Data</button>
